...
\question{We had a nice reference for Schur's test. Do you remember? Was it  Halmos and  Shankar Sunder, Bounded integral operators on $L^2$ spaces}
...
\[
 \kappa(z,w)=\frac1{\pi|1-z\overline w|^2},
\]
use the Schur weight $\chi(z)=(1-|z|^2)^{-1/2}$.  With $z=re^{i\theta}$ and $w=\rho e^{i\phi}$,
\[
...
For illustration, we truncate hyperbolic radius to $0<x<X$, discretize each angular block by a symmetric Nystr\"om method, and order the resulting finite-dimensional singular values globally.  The truncation makes every block compact; its discrete modes should be interpreted as spectral packets sampling the continuous parameter $\mu$, not as convergent eigenvalues of the infinite operator.
\question{I think we need to make it clearer that we use a numerical eigen system routine (eigs) on this Nystrom matrix on a truncated domain. Then we compare with the analytical results}
