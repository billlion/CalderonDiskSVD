...
the linearized  problem have been considered since 
 the 1990s, eg \cite{breckon1990image}.  The problem is noted to be severely illposed with an exponential decay in singular values.  While numerical results give some insight into the severely illposed nature of the problem, and especially the insensitivity to interior perturbations in conductivity, an explicit form of these singular functions has been elusive. Inspired by the explicit singular value decomposition of the Radon on transform on the disk that was already well known in the 1980s, the author, as a naive doctoral student, assumed that it would be fairly straightforward to find such an explicit decomposition for the Calder\'on problem. The realization by the author that normal operator is {\em not} compact (see Prop \ref{prop:noncompact}), in around 2023, offered some explanation for decades of lack of progress, but also gave  a path to  the extensive body of literature on spectral theory of related operators
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{figures/nonradial_singular_functions}
	\caption{
...
\ack{This draft was developed with substantial assistance from ChatGPT (OpenAI), using GPT-5.6 Sol Pro through a ChatGPT Pro subscription, accessed in September 2026.  The assistance included proposing candidate derivations, symbolic and numerical checks, literature-search support and drafting. In partuclar the LLM found the connection to yperbolic geometry and wighted Bergman Berezin transform.  The human author is responsible for checking every argument, computation, citation and originality claim. 
