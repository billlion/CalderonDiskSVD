...
\begin{abstract}
	...
	generalized singular system for the linearized forward map.
...
Calder\'on introduced the inverse boundary problem that now bears his name in the conductivity setting \cite{Calderon1980}, including calculation of the Fr\'echet derivative and a proof of its injectivity.  This is important for medical, industrial and geophysical imaging, where it goes by various names including Electrical Impedance Tomography (EIT) and Electrical Resistivity Tomography  \cite{adler2015electrical}.


 
 The singular value decomposition (SVD) of the matrix of the derivative of the discretized forward problem of  EIT  has played an important role in studying the ill-posedness and non-uniformity of resolution  since the 1980s.  Murai and Kagawa \cite{murai1985electrical} used a numerical SVD for their resistor mesh/FEM model. The Brecon and Pidccock also calculated the SVD using a FEM model and in \cite{breckon1988some} used the singular values to show the relative unimportance of drive pair separation.  Also  in  1988 Connoly and Wall \cite{connolly1988inverse} published a plot of the singular values calculated numerically  for the rotationally symmetric case. Plots of the singular functions calculated numerically for the disk were shown in a  keynote talk ‘What can you see with EIT” by the  author at 3rd EU
 Workshop on EIT Copenhagen in 1990 and also in his PhD thesis \cite{breckon1990image}. 
 
 
 
  The problem is noted to be severely illposed with an exponential decay in singular values.  While numerical results give some insight into the severely illposed nature of the problem, and especially the insensitivity to interior perturbations in conductivity, an explicit form of these singular functions has been elusive. Inspired by the explicit singular value decomposition of the Radon  transform on the disk that was already well known in the 1980s, the author, as a nai\"ve doctoral student, assumed that it would be fairly straightforward to find such an explicit decomposition for the Calder\'on problem. The realization by the author that normal operator is {\em not} compact (see Prop \ref{prop:noncompact}), in around 2023, offered some explanation for decades of lack of progress, but also gave  a path to  the extensive body of literature on spectral theory of related operators
 
  The radial subproblem, ie rotationally invariant unknown, corresponding to the $m=0$ angular Fourier
block, is particularly transparent. For rotationally symmetric perturbations, a multiplicity-normalized normal operator reduces to the Hilbert kernel \cite{Lionheart2026AtLast}. 
\begin{equation}
 (Hf)(t)=\int_0^1\frac{f(s)}{1-st}\,\dd s.
 \label{eq:radial-hilbert}
\end{equation}
Its continuous spectral multiplier may be obtained from an adjacent-interval finite Hilbert transform, from a commuting Sturm--Liouville expression, from Hilbert-matrix theory, or from a P\"oschl--Teller scattering problem; see, for example, \cite{KatsevichTovbis2016,AlemanMontesSarafoleanu2012,MontesVirtanen2025}. The Finite Hilbert transform approach especially gives insight in to the non-compactness.  The finite Hilbert transform is on adjacent intervals, ie sharing one endpoint.  If a gap is introduced, in our case meaning the unknown parameter is known in a neighbourhood of the boundary, then the operator is compact. Similarly if Neumann-to-Dirichlet data is considered compactness is restored. However in both these cases we do not (yet) have such an explicit diagonalization. 

  The purpose of the present paper is to show that the full, non-rotationally symmetric problem is not merely a direct sum of unrelated radial calculations for different frequency modes.  The angular blocks assemble into one hyperbolically invariant operator on the disk and this gives a diagonalization of this non-compact operator. This gives a practically and theoretically satisfying  answer to an important special case of a long-standing problem of importance in applications.
...
We work initially in the Schr\"odinger formulation.  Its relation to
...
\begin{proof}
For $u\in H^1(\D)$ and $v\in H_0^1(\D)$, H\"older's
inequality with exponents $2,4,4$ gives
\[
...
\[
H^1(\D)\hookrightarrow L^4(\D),
\qquad
H_0^1(\D)\hookrightarrow L^4(\D)
\]
therefore imply
\[
\left|\int_{\D}q\,u\overline v\,\dd A\right|
...
\label{sec:spectrum}
The spectrum of $-\Delta_H$ on the hyperbolic plane is
$[1/4,\infty)$ and is purely absolutely continuous
\cite{Helgason1984}.  For $\mu\geq0$, let
\[
\phi_\mu(d)
=
P_{-1/2+i\mu}(\cosh d),
...
\begin{proof}
	...
	Termwise integration of the hypergeometric series yields
	\[
	\begin{aligned}
		\int_0^1
		(1-t)^{a-1}{}_2F_1(a,a;1;t)\,\dd t
...
\begin{figure}
	...
	\caption{
		...
		measurement map.  Each mode is labelled by its angular frequency
		$|m|$, the effective hyperbolic spectral parameter $\mu_\lambda$
		obtained from
		$
		\lambda=\pi/\cosh (\pi \mu_\lambda),
		$
		and its radial zero count.  The modes are ordered globally by the
		computed normal-operator eigenvalue.  The resulting sequence interlaces
		angular and radial complexity.  The increasing concentration of radial
		oscillations near $r=1$ is the Euclidean manifestation of approximately
		uniform oscillation in hyperbolic radius.  For $|m|>0$, a second real
		mode is obtained by replacing the cosine angular factor by a sine
		factor.
}
...
\ack{This draft was developed with substantial assistance from ChatGPT (thanks for the access donated by OpenAI), using GPT-5.6 Sol Pro through a ChatGPT Pro subscription, accessed in September 2026.  The assistance included proposing candidate derivations, symbolic and numerical checks, literature-search support and drafting. In partuclar the LLM found the connection to yperbolic geometry and weighted Bergman Berezin transform.  The human author is responsible for checking every argument, computation, citation and originality claim.

Part of this work was complete while the author was a Visiting Fellow at Clare Hall, Cambridge} 
	% \todo{Revise this disclosure after the final human verification record and author contributions are complete.}}

\roles{William R B Lionheart: conceptualization, methodology, investigation, validation, writing, and supervision. As well as the choice of problem the derivation of the kernel function of the normal operator and the multiplicity weighting was the human author's insight although the LLM was able to reproduce and extend this reasoning with minimal prompting.  }
	% \todo{Replace by the final CRediT statement if further authors join the paper.}}

\data{The MATLAB code used for the exploratory finite-section figures, along with some Mathematica verifications wiil be released on github on a public repository}}
